...
fileprivate extension CGColor {
	...
	static let named: NamedColors = .init()
}
...
public extension UIColor {
	...
	internal convenience init?(hexString: String) {
		...
		if workingString.count == 3 {
			guard let asInt = UInt16(workingString, radix: 16) else { return nil }
			...
			guard let asInt = UInt32(workingString, radix: 16) else { return nil }
		...
		guard colorArray.count == 4 else { return nil }
	...
	convenience init?(cssName: String) {
		guard let namedColor = CGColor.named[cssName.lowercased()] else { return nil }
